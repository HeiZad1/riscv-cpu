...
CFLAGS = -fPIC --timescale "1ns/1ns" --no-timing
...
$(OBJ_DIR)/V$(TOP).mk: $(VERILOG_SRCS) $(CPP_SRCS) | $(OBJ_DIR)
	verilator --timescale "1ns/1ns" --no-timing  -Wall $(INCLUDE_DIRS) $(DEFINES) $(VERILOG_SRCS) --cc --exe $(CPP_SRCS) --CFLAGS "$(CFLAGS)" --LDFLAGS "  $(LDFLAGS)" --trace --top-module $(TOP) 
